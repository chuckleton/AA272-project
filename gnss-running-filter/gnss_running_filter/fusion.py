...
from gnss_running_filter.deltat import DeltaT
import numpy as np
...
class Fusion(object):
    ...
    def calibrate(self, getxyz, stopfunc, wait=0):
        ...
        while not stopfunc():
            ...
            magxyz = getxyz().__next__()
            for x in range(3):
                magmax[x] = max(magmax[x], magxyz[x])
                magmin[x] = min(magmin[x], magxyz[x])
        self.magbias = (np.array(magmin) + np.array(magmax)) / 2.0
